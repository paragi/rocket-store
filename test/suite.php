...
function test($name, $result, $count = null, $must_fail = false){
    ...
    if( (!empty($count) and $result['count'] != $count)
        ...
        print_r($result);
        if($count) echo "Expected a count of {$count}\n";
    }else
        echo "Test: $name - OK\n";
    $test['count']++;
}
...
test("Set data storage area"
    ,["error" => $rs->data_storage_area == $dir . DIRECTORY_SEPARATOR ? "" : "Data storage area directory: $rs->data_storage_area  != '${dir}/'" ]
);
...
test("Create sequence"
    ,["error" => "", "count" => $rs->sequence($collection) ]
    ,1
);
...
test("Post with GUID key only"
   ,$rs->post($collection,"",$record,RS_ADD_GUID)
   ,1
);

test("Post with GUID added to key"
    ,$rs->post($collection,"{$record['name']}",$record,RS_ADD_GUID)
    ,1
);
